...
public class CustomerBase{
    ...
    public static void readCustomers() {
        //read Costs file and store into arraylist of Object(contact)
        ...
        try {
            if (new File(FileName).exists()) {
                ...
                while ((line = br.readLine()) != null) {
                    if (!"".equals(line)) {
                        String[] _temp = line.split(",");
                        if (_temp[1].equalsIgnoreCase(searchValue)) {
    ...
    public static void deleteCustomer(Customer C) {
        Costs c = new Costs(C.getNumber(), C.getCost());
        CostsBase.deleteCost(c);
        BufferedReader br = null;
        String ReWrite = "";
        try {
            if (new File(FileName).exists()) {
                ...
                while ((line = br.readLine()) != null) {
                    String[] _temp = line.split(",");
                    System.out.println(_temp[0] + ","+ _temp[1] + ","+ _temp[2] + ","+ _temp[3]);
                    if (_temp[0].equalsIgnoreCase(C.getName()) && _temp[1].equalsIgnoreCase(C.getNumber())
                            && _temp[2].equalsIgnoreCase(C.getAdress()) && _temp[3].equalsIgnoreCase(C.getCost())) {
